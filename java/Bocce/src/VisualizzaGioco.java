...
public class VisualizzaGioco extends JFrame
{
  ...
  /** Costruisce il visualizzatore caricando il gioco dal file il
   * cui nome e' dato in argomento. */
  public VisualizzaGioco(String nome)
  {
    ...
    getContentPane().add(
        b_esci = new JButton("termina programma"),BorderLayout.SOUTH);
    
    JPanel destro = new JPanel();
    destro.setLayout(new GridLayout(6,1));
    destro.add(b_avvia = new JButton("tira boccino"));
    destro.add(creaPannelloTre(
               lab_tiri = new JLabel("Tiri rimasti: "+gioco.numeroTiri()),
               new JLabel(), 
               lab_punti = new JLabel("Punti: 0  "),
               "gioco"));
    destro.add(creaPannelloTre(
               lab_zoom = new JLabel(),
               s_zoom = creaSlider(ZOOM_MIN, ZOOM_MAX, ZOOM, 1,1),
               b_zoom_reset = new JButton("default"),
               "zoom"));
    destro.add(creaPannelloTre(
               lab_animaz = new JLabel(),
               s_animaz = creaSlider(INTERV_MIN,INTERV_MAX,intervallo=INTERVALLO,5,1),
               b_animaz_reset = new JButton("default"),
               "moviola"));
    destro.add(creaPannelloTre(
               lab_velocita = new JLabel(),
               s_velocita = creaSlider(VELOC_MIN,VELOC_MAX,velocitaBoccino=VELOCITA,15,5),
               b_vel_casuale = new JButton("casuale"),
               "velocita boccino"));
    destro.add(creaPannelloTre(
               lab_angolo = new JLabel(),
               s_angolo = creaSlider(ANG_MIN,ANG_MAX,angoloBoccino=ANGOLO,90,30),
               b_ang_casuale = new JButton("casuale"),
               "angolo boccino"));
  ...
  {
    b_esci.addActionListener(new ActionListener()
    { // termina il programma
      public void actionPerformed(ActionEvent ev)
      {  System.exit(0);  }
    });

    b_avvia.addActionListener(new ActionListener()
    { // imposta la velocita' del boccino e avvia il timer della simulazione
       public void actionPerformed(ActionEvent ev)
       {
         gioco.preparaBoccino((double)velocitaBoccino, 
                              Math.toRadians((double)angoloBoccino));
         aggiornaStato();
         avanzamento.start();
       }
    });

    b_vel_casuale.addActionListener(new ActionListener()
    {   // sceglie una velocita' casuale
       public void actionPerformed(ActionEvent ev)
       {  cambiaIntensita((int)(Math.random()*40.0 + 5.0));  }
    });

    s_velocita.addChangeListener(new ChangeListener()
    {  // sceglie la velocita' dello slider
       public void stateChanged(ChangeEvent e)
       {  cambiaIntensita(s_velocita.getValue());  }
    });

    b_ang_casuale.addActionListener(new ActionListener()
    {  // sceglie un angolo casuale
       public void actionPerformed(ActionEvent ev)
       {  cambiaAngolo((int)(Math.random()*360.0));  }
    });

    s_angolo.addChangeListener(new ChangeListener()
    {  // sceglie l'angolo dello slider
       public void stateChanged(ChangeEvent e)
       {  cambiaAngolo(s_angolo.getValue());  }
    });

    s_zoom.addChangeListener(new ChangeListener()
    {  // imposta lo zoom dello slider
       public void stateChanged(ChangeEvent e)
       {  cambiaZoom(s_zoom.getValue());  }
    });

    b_zoom_reset.addActionListener(new ActionListener()
    {  // reimposta lo zoom iniziale
       public void actionPerformed(ActionEvent ev)
       {  cambiaZoom(ZOOM);
    ...
    s_animaz.addChangeListener(new ChangeListener()
    {  // imposta il ritardo dello slider
       public void stateChanged(ChangeEvent e)
       {  cambiaIntervalloTimer(s_animaz.getValue());  }
    });

    b_animaz_reset.addActionListener(new ActionListener()
    {  // reimposta il ritardo iniziale
       public void actionPerformed(ActionEvent ev)
  ...
  private void cambiaIntervalloTimer(int n)
  {
    ...
    if (avanzamento.isRunning()) avanzamento.restart();
    aggiornaStato();
  }
  
  // crea uno slider dati i valori minimo, massimo, iniziale
  // e gli intervalli ai quali disegnare i trattini
  JSlider creaSlider(int minimo, int massimo, int iniziale,
                     int intervalloMaggiore, int intervalloMinore)
  {
  ...
  JPanel creaPannelloTre(JComponent c1, JComponent c2, JComponent c3, String s)
  {
    ...
    p.setBorder(BorderFactory.createTitledBorder(
        BorderFactory.createLineBorder(Color.gray,2), s));
  ...
  void creaTimer()
  {
    ActionListener avanzaUnPasso = new ActionListener()
    {  // azione da compiere ad ogni scatto del timer
       public void actionPerformed(ActionEvent ev)
       {
          ...
          if (gioco.bocceFerme())
          {
              ...
              if (gioco.giocoFinito())
              {
                JOptionPane.showMessageDialog(VisualizzaGioco.this,
    ...
    avanzamento = new Timer(
         (int)(Gioco.DELTA_T*50.0*intervallo),avanzaUnPasso);
  }
  ...
  protected class PannelloGioco extends JPanel
  {
    ...
    {
      zoom = z;
      setPreferredSize(new Dimension(2*eps+(int)(zoom*gioco.campoX()), 
                       2*eps+(int)(zoom*gioco.campoY())));
    }
  ...
  /** Chiede il livello di difficolta' per il gioco (numero di tiri)
   * mediante una finestra apposita. */
  ...
  /** Se la finestra grafica del visualizzatore e' aperta, 
   * mostra il messaggio in una finestra apposita, altrimenti lo mostra
   * sulla finestra testuale da cui e' stato lanciato il programma.
   * @param e eccezione che rappresenta l'errore avvenuto
   * @param commento commento da mostrare
   * @param termina se vero, dopo mostrato il messaggio deve termina
   * il programma, altrimenti no. */
  public void trattaEccezione(Exception e, String commento, boolean termina)
  {
    String msg = "Errore: "+e.getClass().getName()+": "
       +e.getMessage()+"\n"+commento+"\n";
    ...
    if (termina) System.exit(1);
  }
  
  /** Main: crea il visualizzatore caricando il gioco dal file il cui nome
   * e' stato messo sulla command-line. */
